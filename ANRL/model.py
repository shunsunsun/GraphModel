...
class Model:
    def __init__(self, config, N, dims, X_target):
        ...
        for i in range(self.layers - 1):
            ...
            self.W[name_W] = tf.get_variable(name_W, [struct[i], struct[i+1]], initializer = tf.contrib.layers.xavier_initializer())
        ...
        self.grads_vars_ae = [v for (g, v) in self.grads_all_ae if g is not None]
        self.gradient_ae = self.optimizer_ae.compute_gradients(self.loss_ae, self.grads_vars_ae)
        self.train_opt_ae = self.optimizer_ae.apply_gradients([(tf.clip_by_norm(gv[0], 10), gv[1]) for gv in self.gradient_ae])

        ############ define variables for skipgram  ####################
        # construct variables for nce loss
        self.nce_weights = tf.get_variable("nce_weights", [self.N, self.dims], initializer = tf.contrib.layers.xavier_initializer())
        ...
        self.grads_vars_sg = [v for (g, v) in self.grads_all_sg if g is not None]
        self.gradient_sg = self.optimizer_sg.compute_gradients(self.loss_sg, self.grads_vars_sg)
        self.train_opt_sg = self.optimizer_sg.apply_gradients([(tf.clip_by_norm(gv[0], 10), gv[1]) for gv in self.gradient_sg])
